...
question_pattern = CaselessLiteral("<eqn>").suppress() + ... + CaselessLiteral("''</eqn>").suppress() + Suppress(Optional(White())) + Suppress("<watex>") + ... + Suppress("</watex>") + Suppress(Optional(White())) + Suppress("<_>")
...
def parse_section(section: str, contents: str):
    ...
    if(section == 'choices'):
        try:
            return [clean_text(answer_pattern.parseString(line)[0]) for line in contents.splitlines()]
        except:
            return ['']
...
def parse_webassign(instring: str) -> dict:
    ...
    for line in lines:
        ...
        if line:
            ...
            try:
                currentSection = section_pattern.parseString(line.strip())[0].lower()
            ...
            # otherwise, continue to add to current section
                ...
                if currentSection:
                    ...
                    # preserve linen breaks in these sections
                    else:
                        return_dict[currentSection] = return_dict[currentSection] + line + '\n'
    ...
    for key in deepcopy(return_dict).keys():
        if key == 'question':
            code_contents, statement_contents = question_pattern.parseString(return_dict[key])
            return_dict['parameters'] = {
                'code': code_contents
            }
            return_dict['statement'] = parse_section('statement', statement_contents)
        if key == 'answer':
            return_dict['choices'] = parse_section('choices', return_dict['answer'])
        if key == 'solution':
            return_dict['solution'] = parse_section('solution', solution_pattern.parseString(return_dict['solution'])[0])
        if key == 'info':
