...
unary_operators = ['H', 'S', 'const', 'exp', 'trig', 'poly', 'linear', 'quadratic', 'cubic', 'quartic', 'quintic', 'sqrt', 'cbrt',
                   'neg', 'abs', 'log', 'ln', 'sin', 'cos', 'tan', 'sec', 'csc', 'cot', 'arcsin', 'arccos', 'arctan', 'arcsec', 'arccsc', 'arccot']
...
def check_commas(string):
    ...
    for i in log_indices:
        ...
        comma_count = sum([1 for i in range(len(context[0])) if (
            context[0][i] == ',' and get_context(context[0], i)[0] == context[0])])
        if comma_count > 1:
...
class MathString:
    ...
    def add_times(self):
        ...
        while i < len(self.s):
            if self.s[i] == ' ':
                ws = get_adjacent_characters(
                    self.s, i, test=lambda x: x == ' ')
                if ws[1] == 0:
